...
void list_insert(node* previous_ptr, const node::coef_t& new_coefficient, const node::power_t& new_exponent) {
	previous_ptr->set_link(new node(new_coefficient, new_exponent, previous_ptr->get_link()));
}
...
void list_copy(const node* source_ptr, node*& head_ptr, node*& tail_ptr) {
	...
	list_head_insert(head_ptr, source_ptr->get_coefficient(), source_ptr->get_exponent());
	tail_ptr = head_ptr;
	source_ptr = source_ptr->get_link();
	while (source_ptr != NULL) {
		list_insert(tail_ptr, source_ptr->get_coefficient(), source_ptr->get_exponent());
